...
header("Backlog")
all_releases = get(f"http://api:8101/get_objects/release?project_id={current_project.split(':')[0]}")

for release in all_releases.json():
    all_reqs = get(f"http://api:8101/get_objects/requirement?parent={release['shortname']}").json()
