...
def find_parent(search_term):
    ...
    api_call_result = get(f"http://api:8101/get_objects/{parent_object.__name__.lower()}")
    print(api_call_result)
    return [f"{db_object['shortname']}: {db_object['title']}" for db_object in api_call_result.json()
...
object_type = selectbox(label="Select object type",
                        key="object_type",
...
if object_type:
    form_dict["title"] = text_input(label="Title", key="title", placeholder=f"Insert {object_type.lower()} title")
    ...
    form_dict["description"] = text_area(label="Description",
                                         key="description",
    ...
    if button(label="Submit"):
        ...
        return_value = get(f"http://api:8101/insert_object/{object_type.replace(' ', '').lower()}", params=form_dict)
