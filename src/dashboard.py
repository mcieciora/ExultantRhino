...
with requirements:
    metric(
        ...
        value=len(
            get(
                f"http://api:8101/get_objects/requirement?project_id="
...
with testcases:
    metric(
        ...
        value=len(
            get(
                f"http://api:8101/get_objects/testcase?project_id={current_project.split(':')[0]}",
                timeout=5,
            ).json()
        ),
    )
...
with bugs:
    metric(
        ...
        value=len(
            get(
                f"http://api:8101/get_objects/bug?project_id={current_project.split(':')[0]}",
                timeout=5,
            ).json()
        ),
    )
