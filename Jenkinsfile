        ...
        stage ("Code analysis") {
            ...
            parallel {
                stage ("pylint") {
                    steps {
                        script {
                            sh "docker run --rm test_image python -m pylint src --max-line-length=120 --disable=C0103,C0114 --fail-under=9.5"
                ...
                stage ("Code coverage") {
                    steps {
                        script {
                            sh "docker run --name code_coverage_container test_image python -m pytest --cov=src automated_tests/unittest --cov-fail-under=85 --cov-config=automated_tests/.coveragerc --cov-report=html"
                        }
                    }
                    post {
                        always {
